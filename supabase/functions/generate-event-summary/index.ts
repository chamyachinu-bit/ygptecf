...
serve(async (req) => {
  try {
    ...
    const summary = [
      ...
      '',
      'BUDGET',
      `Estimated: INR ${totalEstimated.toFixed(2)}`,
      `Actual: INR ${totalActual.toFixed(2)}`,
      `Variance: INR ${variance.toFixed(2)} (${variancePct}%)`,
      `Donations Received: INR ${Number(report?.donations_received ?? 0).toFixed(2)}`,
      '',
      'APPROVALS',
